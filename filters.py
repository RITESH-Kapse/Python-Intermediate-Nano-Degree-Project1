...
class AttributeFilter:
    ...
    def __repr__(self):
        """Retrun the class name and operatir name with its value."""
        return f"{self.__class__.__name__}(op=operator.{self.op.__name__}, \
            value={self.value})"

# RiteshComments -- As suggested creating classes and class methods
# to pass the filtered output from function create_filters.


class DateFilter(AttributeFilter):
    """Get time filtered results from close approach."""

    @classmethod
    def get(cls, approach):
        """Return approach.time with .date() method."""
        return approach.time.date()


class HazardousFilter(AttributeFilter):
    """Get filtered results from close approach for hazardous field."""

    @classmethod
    def get(cls, approach):
        """Return neo.hazardous with approach objects."""
        return approach.neo.hazardous


class DiameterFilter(AttributeFilter):
    """Get diameter filtered results from close approach."""

    @classmethod
    def get(cls, approach):
        """Return neo.diameter with approach objects."""
        if str(approach.neo.diameter) == 'nan':
            raise UnsupportedCriterionError
        else:
            return approach.neo.diameter


class DistanecFilter(AttributeFilter):
    """Get distance filtered results from close approach."""

    @classmethod
    def get(cls, approach):
        """Return distance from approach objects."""
        return approach.distance


class VelocityFilter(AttributeFilter):
    """Get velocity filtered results from close approach."""

    @classmethod
    def get(cls, approach):
        """Return velocity from approach objects."""
...
def create_filters(date=None, start_date=None, end_date=None,
    ...
    a value from the user's options at the command line. Each
    one corresponds to a different type of filter. For example,
